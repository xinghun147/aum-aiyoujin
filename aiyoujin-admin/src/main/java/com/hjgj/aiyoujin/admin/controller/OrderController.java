...
import com.hjgj.aiyoujin.admin.common.excel.ExcelUtil;
...
public class OrderController extends BaseController {
    ...
    @RequestMapping(value = "/buyOrderExcel")
    public void buyOrderExport(OrderRequestVo requestVo, Integer pageNum, Integer pageSize, HttpServletResponse response) {
        pageNum = pageNum == null ? super.pageNum : pageNum;
        Page<OrderBuyExportVo> orderVOMap = adminOrderService.getBuyOrderExportMap(requestVo, pageNum, 5000);
        try {
            Map<String, String> map = new HashMap<String, String>();
            map.put("title", "买入订单表");
            map.put("total", orderVOMap.getTotal() + " 条");
            map.put("date", DateUtil.getTime());
            ExcelUtil.getInstance().exportObj2ExcelByTemplate("买入订单表", map, "OrderBuyInfo.xls", orderVOMap.getList(), OrderBuyExportVo.class, true, response);
        } catch (IOException e) {
            logger.error("导出买入订单表到Excel异常：", e);
        }
    }

    @RequestMapping(value = "/sellOrderExcel")
    public void sellOrderExport(OrderRequestVo requestVo, Integer pageNum, Integer pageSize, HttpServletResponse response) {
        pageNum = pageNum == null ? super.pageNum : pageNum;
        Page<OrderSellExportVo> orderVOMap = adminOrderService.getSellOrderExportMap(requestVo, pageNum, 5000);
        try {
            Map<String, String> map = new HashMap<String, String>();
            map.put("title", "变现订单表");
            map.put("total", orderVOMap.getTotal() + " 条");
            map.put("date", DateUtil.getTime());
            ExcelUtil.getInstance().exportObj2ExcelByTemplate("变现订单表", map, "OrderSellInfo.xls", orderVOMap.getList(), OrderSellExportVo.class, true, response);
        } catch (IOException e) {
            logger.error("导出变现订单表到Excel异常：", e);
        }
    }

    @RequestMapping(value = "/sendOrderExcel")
    public void sendOrderExport(OrderRequestVo requestVo, Integer pageNum, Integer pageSize, HttpServletResponse response) {
        pageNum = pageNum == null ? super.pageNum : pageNum;
        Page<OrderSendExportVo> orderVOMap = adminOrderService.getSendOrderExportMap(requestVo, pageNum, 5000);
        try {
            Map<String, String> map = new HashMap<String, String>();
            map.put("title", "转送订单表");
            map.put("total", orderVOMap.getTotal() + " 条");
            map.put("date", DateUtil.getTime());
            ExcelUtil.getInstance().exportObj2ExcelByTemplate("转送订单表", map, "OrderSendInfo.xls", orderVOMap.getList(), OrderSendExportVo.class, true, response);
        } catch (IOException e) {
            logger.error("导出转送订单表到Excel异常：", e);
        }
    }

    @RequestMapping(value = "/pickOrderExcel")
    public void pickOrderExport(OrderRequestVo requestVo, Integer pageNum, Integer pageSize, HttpServletResponse response) {
        pageNum = pageNum == null ? super.pageNum : pageNum;
        Page<OrderPickExportVo> orderVOMap = adminOrderService.getPickOrderExportMap(requestVo, pageNum, 5000);
        try {
            Map<String, String> map = new HashMap<String, String>();
            map.put("title", "提货订单表");
            map.put("total", orderVOMap.getTotal() + " 条");
            map.put("date", DateUtil.getTime());
            ExcelUtil.getInstance().exportObj2ExcelByTemplate("提货订单表", map, "OrderPickInfo.xls", orderVOMap.getList(), OrderPickExportVo.class, true, response);
        } catch (IOException e) {
            logger.error("导出提货订单表到Excel异常：", e);
        }
    }
}